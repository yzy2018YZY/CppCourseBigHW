...
int main(int argc, char** argv)
{
	...
	//char test_choise[1024] =   "000000000001";
	char tmp[80];
	int i, round;
	...
	if (test_choise[T_RECTANGLE] == '1') {
		...
		for (round = 0; round <= 1; round++)
			for (i = 0; i < 360; i += 15) {
				...
				//				hdc_rectangle(350, 350, 300 - round*30, 200 - round * 20, i, round, 1+rand()%5, RGB(rand()%256, rand()%256, rand()%256));	//随机色
	...
	if (test_choise[T_SQUARE] == '1') {
		...
		for (round = 0; round <= 1; round++)
			for (i = 0; i < 360; i += 15) {
				...
				to_be_continued(tmp, 0, 0);
				//				hdc_square(350, 350, 300 - round * 30, i, round, 2+rand()%5, RGB(rand() % 256, rand() % 256, rand() % 256)); //随机色
	...
	if (test_choise[T_SECTOR] == '1') {
		...
		for (round = 0; round <= 1; round++)
			for (i = 0; t[i].angle_begin != -1 && t[i].angle_end != -1; i++) {
				...
				//				hdc_sector(350, 350, 200 - round * 20 + i * 10, t[i].angle_begin, t[i].angle_end, round, 2+rand()%5, RGB(rand() % 256, rand() % 256, rand() % 256)); //随机色
	...
	if (test_choise[T_PEPPAPIG] == '1') {
		...
		to_be_continued("画小猪佩奇完成", STL_X, STL_Y);
	}

	if (test_choise[T_MATHCURVE] == '1') {
		/* 画一条数学曲线，具体内容可自选（例：阿基米德螺线）
		   必做项 */
		hdc_cls();
		for (round = 1; round <= 5; round++)
		{
			double a = round * 10, b = round * 0.1;
			sprintf(tmp, "参数：a = %g , b = %g , 角度 = %d", a, b, 720);
			hdc_draw_Mathematical_curve(400, 400, 0, 720, a, b, 12, color[(round % 2) ? 0 : 2]);
			to_be_continued(tmp, 0, 0);
			hdc_cls();
		}
		to_be_continued("画阿基米德螺线完成", STL_X, STL_Y);
	}
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		hdc_cls();
		hdc_draw_Eevee(500, 400);
		to_be_continued("画伊布完成", STL_X, STL_Y);
	}

	hdc_release();
	gotoxy(STL_X, STL_Y - 2);
