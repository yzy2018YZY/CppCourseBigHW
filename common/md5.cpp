...
void MD5(char *dst, const char *src)
{
	...
	MD5Final(&md5, encryptd);

	/* MD5是一个128bit的串（16字节存放），按字节理解不保证每字节都是可见图形字符，为了方便文本查看，转换为hex(每字节的ASCII码转两字节的hex)，长度32字节 */
	for (i = 0; i < 16; i++)
		s += sprintf(s, "%02x", int(encryptd[i]));
	return;
}
